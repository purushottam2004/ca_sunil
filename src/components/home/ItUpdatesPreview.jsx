...
import { itUpdates } from '@/data/itUpdates'
import AnimatedSection from '@/components/ui/AnimatedSection'
import SectionHeading from '@/components/ui/SectionHeading'
import Badge from '@/components/ui/Badge'

export default function ItUpdatesPreview() {
  const latest = itUpdates.slice(0, 3)
    ...
    <section className="py-20 bg-white dark:bg-gray-900">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <AnimatedSection className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-10">
          <SectionHeading
            eyebrow="Income Tax News"
            title="Latest Income Tax Updates"
            subtitle="Budget changes, TDS amendments, ITR updates, and new tax rules."
          />
          <Link to="/income-tax-updates" className="inline-flex items-center gap-1 text-brand-gold font-semibold hover:gap-2 transition-all whitespace-nowrap">
          ...
          {latest.map((update, i) => (
            <AnimatedSection key={update.id} delay={i * 0.1}>
              <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 h-full border border-gray-100 dark:border-gray-700 hover:shadow-lg transition-all duration-300 hover:-translate-y-0.5">
                <Badge color="green" className="mb-3">{update.category}</Badge>
