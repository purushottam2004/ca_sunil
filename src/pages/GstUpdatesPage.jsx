...
import { gstUpdates, gstCategories } from '@/data/gstUpdates'
...
export default function GstUpdatesPage() {
  ...
  const filtered = gstUpdates.filter(u => {
    ...
    <div className="bg-white dark:bg-gray-900 min-h-screen">
      ...
      <section className="bg-gradient-to-br from-brand-navy via-blue-950 to-slate-900 text-white py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <p className="text-brand-gold text-sm font-semibold uppercase tracking-widest mb-3">Always Updated</p>
          <h1 className="text-4xl md:text-5xl font-bold mb-4">GST Updates</h1>
          <p className="text-gray-300 text-lg max-w-2xl mx-auto">
            Latest GST circulars, notifications, e-invoice updates, and compliance changes explained in plain language.
          </p>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <AnimatedSection>
            <div className="flex flex-col sm:flex-row gap-4 mb-8">
              <SearchBar
                onSearch={setQuery}
                placeholder="Search GST updates..."
                className="flex-1"
              />
            </div>
            {/* Category chips */}
              ...
              {filtered.map((update, i) => (
                <AnimatedSection key={update.id} delay={(i % 6) * 0.07}>
                  <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 h-full border border-gray-100 dark:border-gray-700 hover:shadow-lg transition-all duration-300 hover:-translate-y-0.5 flex flex-col">
                    <Badge color="blue" className="mb-3 self-start">{update.category}</Badge>
